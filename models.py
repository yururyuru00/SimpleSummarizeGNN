...
class SummarizeGCN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.in_encoder = Linear(cfg.n_feat, cfg.n_hid)
    ...
    def forward(self, x, edge_index):
        ...
        for l, conv in enumerate(self.convs):
            ...
            hs.append(x)
            ws.append(conv.conv.lin.weight.detach()) # ws will be [W^1, W^2, ..., W^L]

        alpha, h = self.attention_skip(hs, ws)  # hs = [h^1,h^2,...,h^L], each h^l is (n, d).
        return alpha, self.out_lin(h)
...
class SummarizeGAT(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.in_encoder = Linear(cfg.n_feat, cfg.n_hid*cfg.n_head)
        self.convs = torch.nn.ModuleList()
        in_conv = GNNConv(cfg.task, 'gat_conv', cfg.n_hid, cfg.n_hid, cfg.norm,
    ...
    def forward(self, x, edge_index):
        ...
        for l, conv in enumerate(self.convs):
            ...
            x = F.elu(x)
            hs.append(x)
            ws.append(conv.conv.lin_src.weight.detach()) # ws will be [W^1, W^2, ..., W^L]
...
class SummarizeSAGE(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        for l, conv in enumerate(self.convs):
            ...
            x = F.dropout(x, self.dropout, training=self.training)
            hs.append(x)
            ws.append(conv.conv.lin_l.weight.detach()) # ws will be [W^1, W^2, ..., W^L]
